...
def rawdata2pkl4bert(path, att_list, dataset_name, percentage):
    ...
    with open(path, 'r') as f:
        ...
        if titles:
            ...
            print([tokenizer.convert_ids_to_tokens(i) for i in values[:3]])
            df = pd.DataFrame(
                {'titles': titles, 'attributes': attributes, 'values': values, 'tags': tags, 'split': splits},
                index=range(len(titles)))
            print(df.shape)
            df['x'] = df['titles'].apply(X_padding)
            df['y'] = df['tags'].apply(X_padding)
            ...
            # TO-DO: Replace the random split with the known split (Change processing(?))
            # index = list(range(len(titles)))
            # random.shuffle(index)
            ...
            test_x = np.asarray(list(test['x'].values))
            test_att = np.asarray(list(test['att'].values))
            #test_value = np.asarray(list(test['values'].values)) - Unclear if it is used
            test_y = np.asarray(list(test['y'].values))

            #att_name = att_name.replace('/', '_')

            #Check if the directory exists, if not, create it
            #if not os.path.exists('../data/{}_{}'.format(dataset_name, percentage)):
            #    os.makedirs('../data/{}_{}'.format(dataset_name, percentage))

            with open('../data/{}_{}.pkl'.format(dataset_name, percentage), 'wb') as outp:
...
if __name__ == '__main__':
    ...
    rawdata2pkl4bert(path, att_list, dataset_name, percentage)
